...
// Extended reinforcement for system prompt — includes behavior pattern
// instructions that subagents need to follow consistently.
function systemPromptInstruction(mode) {
  return [
    '<caveman mode="' + mode + '">',
...
export const CavemanPlugin = async (_ctx) => ({
  ...
  // ── caveman injection into ALL sessions (orchestrator + subagents) ──
  //
  // This hook fires when any opencode session builds its system prompt.
  // When caveman mode is active we append detailed behavior instructions.
  //
  // For the orchestrator session this complements the per-turn reinforcement
  // from tui.prompt.append. For subagent sessions (created via task tool
  // delegation) this is the primary injection point — without it the
  // subagent would have no awareness of the active caveman mode.
  //
  // Why the system prompt and not the delegation prompt argument:
  //   experimental.chat.system.transform reaches ALL subagent types
  //   (explore, fixer, librarian, oracle, designer) uniformly regardless
  //   of how opencode creates the child session. Modifying the task tool's
  //   prompt argument would miss internal agent sessions and be fragile
  //   across opencode versions.
  'experimental.chat.system.transform': async (_input, output) => {
    const active = readFlag(flagPath);
    if (active && !INDEPENDENT_MODES.has(active)) {
      // Append at the end so it acts as a closing instruction.
      // Using XML-style block to make it parseable by subagent models.
      output.system.push(systemPromptInstruction(active));
    }
  },
});
