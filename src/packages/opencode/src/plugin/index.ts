  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("Plugin.state")(function* (ctx) {
        ...
        for (const hook of hooks) {
          for (const key of Object.keys(hook)) hookCounts.set(key, (hookCounts.get(key) ?? 0) + 1)
        }
        for (const [key, count] of hookCounts) {
          if (count > 1)
            yield* Effect.logWarning("multiple plugins registered the same hook", { hook: key, count })
        }
    ...
    >(name: Name, input: Input, output: Output) {
      ...
      for (const hook of s.hooks) {
        ...
        yield* Effect.tryPromise({
          ...
          Effect.catch((error) =>
            Effect.logWarning("plugin hook failed", { hook: name, error: errorMessage(error) }),
          ),
        )
      }
      return output
